...
*This lecture includes many contributions from NYU Prof. Tega Brain, friend and collaborator.*

**When we made [drawing programs](https://github.com/golanlevin/lectures/tree/master/lecture_drawing), we explored the interactivity of a single person (typically), who provides inputs to and receives impressions from a responsive algorithmic system. This is the solypsistic "interactivity of the only child", god of their sandbox. We now consider interactive systems that place the participant/guest/user/subject into conversation *with another mind*. In creating a communication system between multiple interlocutors, we establish a new interactivity: that between siblings, lovers, neighbors, strangers.**
